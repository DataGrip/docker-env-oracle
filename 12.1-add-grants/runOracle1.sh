...
if [[ "$MEM_LIMIT" =~ ^[0-9]+$ ]] && [ "$MEM_LIMIT" -lt "$MIN_MEM" ]; then
    echo "Error: The container doesn't have enough memory allocated."
    echo "A database container needs at least 2 GB of memory."
    echo "You currently only have $((MEM_LIMIT/1024/1024/1024)) GB allocated to the container."
    exit 1
fi
